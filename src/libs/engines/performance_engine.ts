...
export default class PerformanceEngine {
  ...
  getResourceUtilization(resource: Resource): number {
    ...
    for (let i = 0; i < relationships.length; i++) {
      ...
      const processingTime =
        behaviour instanceof InternalBehaviour
          ? this.getProcessingTime(behaviour as InternalBehaviour)
          : 0;
    ...
    if (utilization / capacity >= 1) {
      throw new Error(
        `Resource ${resource.getName()}: Utilization is greater than or equal to 100%. This is due to a long service time`,
      );
    }
  ...
  getProcessingTime(behaviour: InternalBehaviour): number {
    ...
    for (let i = 0; i < relationships.length; i++) {
      ...
      const responseTime =
        element instanceof InternalBehaviour
          ? this.getResponseTime(element as InternalBehaviour)
          : 0;
  ...
  getResponseTime(behaviour: InternalBehaviour): number {
    ...
    if (resource) {
      responseTime =
        this.getProcessingTime(behaviour) / (1 - this.getResourceUtilization(resource as Resource));
    } else {
  ...
  getAllPerformanceMetrics(): Array<QuantitativeMetric> {
    ...
    for (let i = 0; i < internalBehaviours.length; i++) {
      ...
      if (resource) {
        ...
        const externalBehaviour =
          outRelationships.length === 1 ? outRelationships[0].getTarget() : null;
