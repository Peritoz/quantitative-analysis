...
export default class QuantitativeAnalysisEngine {
  ...
  getAllMetrics(includeNormalizedValues: boolean = false): Array<QuantitativeMetric> {
    const workloadMetrics: Array<Partial<QuantitativeMetric>> =
      this.workloadEngine.getAllWorkloadsMetrics();
    ...
    for (let i = 0; i < workloadMetrics.length; i++) {
      ...
      let metric: QuantitativeMetric | undefined = metrics.find(
        m =>
          m.internalBehaviour === workloadMetric.internalBehaviour &&
  ...
  getAllMetricsAsCsv(
    separator: string = ';',
    includeNormalizedValues: boolean = false,
  ): Array<string> {
    const sep = separator;
    const metrics: Array<QuantitativeMetric> = this.getAllMetrics(includeNormalizedValues);
    let response: Array<string> = [
      `resource${sep}service${sep}wLoad${sep}procTime${sep}respTime${sep}util`,
    ];
