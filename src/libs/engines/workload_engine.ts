...
class WorkloadEngine {
  ...
  getAllWorkloadsMetrics(): Array<Partial<QuantitativeMetric>> {
    ...
    for (let i = 0; i < externalBehaviours.length; i++) {
      ...
      const internalBehaviour =
        inRelationships.length === 1 ? inRelationships[0].getSource() : null;
      ...
      if (internalBehaviour instanceof InternalBehaviour) {
        ...
        const resources = internalBehaviorInRelationships.filter(
          r => r.getSource() instanceof Resource,
        );
